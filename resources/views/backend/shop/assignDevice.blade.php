...
@section ('title', trans('labels.backend.shop.management') . ' | ' . trans('labels.backend.restaurant.assignDevice'))
...
@section('page-header')
    <h1>
        ...
        <small>{{ trans('labels.backend.restaurant.assignDevice') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::model($shop, ['route' => ['admin.shop.assignDeviceStore', $shop], 'class' => 'form-horizontal', 'id'=>'assign_card_form', 'role' => 'form', 'method' => 'post']) }}

    {{ Form::hidden('shop_id', $shop->id, ['id' => 'shop_id']) }}
    <div class="box box-success">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.restaurant.assignDevice') }}</h3>
    ...
    <div class="box box-info">
        <div class="box-body">
            ...
            <div class="pull-right">
                <button class="btn btn-success btn-xs">{{trans('labels.backend.restaurant.assignDevice')}}</button>
            </div><!--pull-right-->
    ...
    <script>
        $(function() {
            var table = $('#card-table').DataTable({
                ...
                autoWidth: false,
                ajax: {
                    url: '{{ route("admin.shop.getDevices") }}',
                    type: 'get',
                    data: {shop_id: $('#shop_id').attr("value")},
