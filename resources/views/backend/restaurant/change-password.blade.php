...
@section ('title', trans('labels.backend.restaurant.management') . ' | ' . trans('labels.backend.restaurant.change_password'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.restaurant.management') }}
        <small>{{ trans('labels.backend.restaurant.change_password') }}</small>
    </h1>
@endsection

@section('content')
    {{ Form::open(['route' => ['admin.restaurant.change-password.post', $restaurant, $restaurantUser], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'patch']) }}

        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">{{ trans('labels.backend.restaurant.change_password_for', ['user' => $restaurant->name]) }}</h3>
        ...
        <div class="box box-info">
            <div class="box-body">
                <div class="pull-left">
                    {{ link_to_route('admin.restaurant.accounts', trans('buttons.general.cancel'), [$restaurant], ['class' => 'btn btn-danger btn-xs']) }}
                </div><!--pull-left-->
