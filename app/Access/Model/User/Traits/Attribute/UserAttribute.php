...
namespace App\Access\Model\User\Traits\Attribute;
...
trait UserAttribute
{
    ...
    {
        return '<a href="'.route('admin.access.user.show', $this, false).'" class="btn btn-xs btn-info"><i class="fa fa-search" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.general.crud.view').'"></i></a> ';
    }
    ...
    {
        return '<a href="'.route('admin.access.user.edit', $this, false).'" class="btn btn-xs btn-primary"><i class="fa fa-pencil" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.general.crud.edit').'"></i></a> ';
    }
    ...
    {
        return '<a href="'.route('admin.access.user.change-password', $this, false).'" class="btn btn-xs btn-info"><i class="fa fa-refresh" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.change_password').'"></i></a> ';
    }
    ...
    public function getDeleteButtonAttribute()
    {
        if ($this->id != access()->id() && $this->id != 1) {
            return '<a href="'.route('admin.access.user.destroy', $this, false).'"
                 data-method="delete"
    ...
    {
        return '<a href="'.route('admin.access.user.restore', $this, false).'" name="restore_user" class="btn btn-xs btn-info"><i class="fa fa-refresh" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.restore_user').'"></i></a> ';
    }
    ...
    {
        return '<a href="'.route('admin.access.user.delete-permanently', $this, false).'" name="delete_user_perm" class="btn btn-xs btn-danger"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.delete_permanently').'"></i></a> ';
    }
        ...
        if (! session()->has('admin_user_id') || ! session()->has('temp_user_id')) {
            ...
            if ($this->id != access()->id()) {
                return '<a href="'.route('admin.access.user.login-as',
                    $this, false).'" class="btn btn-xs btn-success"><i class="fa fa-lock" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.login_as',
    ...
    public function getClearSessionButtonAttribute()
    {
        if ($this->id != access()->id() && config('session.driver') == 'database') {
            return '<a href="'.route('admin.access.user.clear-session', $this, false).'"
			 	 data-trans-button-cancel="'.trans('buttons.general.cancel').'"
